jQuery(function ($) {
	...
	function fillAddressFields(data) {
		...
		Object.entries(field_mapping_arr).forEach(([fieldId, userKey]) => {
			// Skip country/state — handled later
			if (
			...
			if (value !== undefined && value !== null) {
				// Fill the billing field
				$(`#${fieldId}`).val(value);

				// Also fill the corresponding shipping field (if exists)
				if (fieldId.startsWith('billing_')) {
					const shippingFieldId = fieldId.replace('billing_', 'shipping_');
					if ($(`#${shippingFieldId}`).length > 0) {
						$(`#${shippingFieldId}`).val(value);
					}
				}
			}
		});

		// Hardcoded values for country/state
		$('#billing_country, #shipping_country').val('CO').trigger('change');
		$('#billing_state, #shipping_state').val('CO-TOL').trigger('change');
	}
