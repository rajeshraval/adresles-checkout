...
class Adresles_Order_Email {
    ...
    public function add_adresles_checkout_to_email_block( $order, $sent_to_admin, $plain_text, $email ) {
        ...
        if ( $adresles_selected || $gift_selected ) {
            ...
            if ( $gift_selected ) {
                ...
                echo '<tr><th style="text-align:left;">' . __( 'Apellido', 'adresles' ) . '</th><td>' . esc_html( get_post_meta( $order->get_id(), '_gift_lastname', true ) ) . '</td></tr>';
                echo '<tr><th style="text-align:left;">' . __( 'Teléfono Móvil', 'adresles' ) . '</th><td>' . esc_html( get_post_meta( $order->get_id(), '_gift_phone', true ) ) . '</td></tr>';
    ...
    public function save_adresles_checkout_order_meta( $order_id, $data ) {
        ...
        if ( isset( $_POST['adresles_gift_selected'] ) && $_POST['adresles_gift_selected'] === '1' ) {
            ...
            update_post_meta( $order_id, '_gift_name', sanitize_text_field( $_POST['gift_name'] ) );
    ...
    public function display_order_details_on_thankyou( $order ) {
        ...
        if ( get_post_meta( $order->get_id(), '_adresles_gift_selected', true ) === 'yes' ) {
            ...
            echo '<tr><th>' . __( 'Nombre', 'adresles' ) . '</th><td>' . esc_html( get_post_meta( $order->get_id(), '_gift_name', true ) ) . '</td></tr>';
            echo '<tr><th>' . __( 'Apellido', 'adresles' ) . '</th><td>' . esc_html( get_post_meta( $order->get_id(), '_gift_lastname', true ) ) . '</td></tr>';
            echo '<tr><th>' . __( 'Teléfono Móvil', 'adresles' ) . '</th><td>' . esc_html( get_post_meta( $order->get_id(), '_gift_phone', true ) ) . '</td></tr>';
            echo '<tr><th>' . __( 'Nota', 'adresles' ) . '</th><td>' . nl2br( esc_html( get_post_meta( $order->get_id(), '_gift_note', true ) ) ) . '</td></tr>';
        }
    ...
    public function display_admin_order_meta_box( $order ) {
        ...
        if ( get_post_meta( $order->get_id(), '_adresles_gift_selected', true ) === 'yes' ) {
            ...
            echo '<p><strong>Nombre:</strong> ' . esc_html( get_post_meta( $order->get_id(), '_gift_name', true ) ) . ' ' . esc_html( get_post_meta( $order->get_id(), '_gift_lastname', true ) ) . '</p>';
            echo '<p><strong>Teléfono Móvil:</strong> ' . esc_html( get_post_meta( $order->get_id(), '_gift_phone', true ) ) . '</p>';
            echo '<p><strong>Nota:</strong> ' . nl2br( esc_html( get_post_meta( $order->get_id(), '_gift_note', true ) ) ) . '</p>';
        }
